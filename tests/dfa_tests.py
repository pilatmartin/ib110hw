...
def test_remove_transition() -> None:
    t1 = {"s": {"a": "s"}}
    automaton = DFA({"s"}, {"a", "b"}, "s", {"s"}, t1)
    assert not automaton.remove_transition("s", "b")
    assert not automaton.remove_transition("not_exists", "a")
    assert automaton.remove_transition("s", "a")
    assert not automaton.transitions["s"]
    assert not automaton.remove_transition("s", "a")

    t2 = {"s": {"a": "s", "b": "s"}}
    automaton.transitions = t2
    assert not automaton.remove_transition("s", "c")
    assert automaton.remove_transition("s", "a")
    assert len(automaton.transitions.keys()) == 1
    assert len(automaton.transitions["s"].keys()) == 1
    assert automaton.remove_transition("s", "b")
    assert not automaton.transitions["s"]


def test_add_state() -> None:
    automaton = DFA({"s1"}, set(), "s1", {"s1"}, {})
    ...
    assert "s2" not in automaton.transitions
